...
clean:
	docker-compose -p csproject_patients down --rmi local --volumes
...
setup-local: check_network
	docker-compose -p csproject_patients down --rmi local --volumes
	docker-compose -f docker-compose-local.yml -p csproject_patients up -d --build
	docker exec -it patients_api composer install

setup-dev: check_network
	docker-compose -p csproject_patients down --rmi local --volumes
	docker-compose -f docker-compose-dev.yml -p csproject_patients up -d --build
	docker exec -it patients_api composer install
